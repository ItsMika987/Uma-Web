...
import { supportCards } from '$lib/data/supportCards';
...
// 🃏 Owned Support Card IDs
function loadSupportRoster(): string[] {
	if (typeof localStorage === 'undefined') return [];
	try {
		const saved = localStorage.getItem('uma_support_roster');
...
export const supportRoster: Writable<string[]> = writable<string[]>(loadSupportRoster());

supportRoster.subscribe((val) => {
	if (typeof localStorage !== 'undefined') {
		localStorage.setItem('uma_support_roster', JSON.stringify(val));
	}
});

export const scoutedSupports = derived(supportRoster, ($r) =>
	supportCards
		.filter((c) => $r.includes(c.id))
		.sort((a, b) => b.rarity - a.rarity || a.name.localeCompare(b.name))
);

export const unscoutedSupports = derived(supportRoster, ($r) =>
	supportCards
		.filter((c) => !$r.includes(c.id))
		.sort((a, b) => b.rarity - a.rarity || a.name.localeCompare(b.name))
);

export function scoutSupport(id: string) {
	supportRoster.update((current) => (current.includes(id) ? current : [...current, id]));
}

// 🎲 Support card pity counters
let sPullCount = 0;
let sPullSinceTwo = 0;

export function randomSupportScout(): SupportCard {
	sPullCount++;
	sPullSinceTwo++;
	...
	if (sPullCount >= 25) {
		...
		sPullCount = 0;
		sPullSinceTwo = 0;
	} else if (sPullSinceTwo >= 2 && sPullSinceTwo <= 5) {
		const rollForTwo = Math.random() < 1 / (6 - sPullSinceTwo);
		rarity = rollForTwo ? 2 : 1;
		if (rarity === 2) sPullSinceTwo = 0;
	} else if (sPullSinceTwo > 5) {
		rarity = 2;
		sPullSinceTwo = 0;
	} else {
		rarity = 1;
	...
	const pool = _supportRarityPools[rarity];
	const source = pool && pool.length > 0 ? pool : _supportRarityFallback;
	const pick = source[Math.floor(Math.random() * source.length)];
	scoutSupport(pick.id);
	return pick;
}
...
function loadPity(): { pullCount: number; pullSinceTwo: number } {
	...
	try {
		...
		return { pullCount: 0, pullSinceTwo: 0 };
	}
}
...
export function randomScout(): Uma {
	...
	const pool = _rarityPools[rarity];
	const fallback = _rarityFallback;
...
const _rarityPools: Record<number, Uma[]> = {};
for (const c of characters) {
	if (!_rarityPools[c.rarity]) _rarityPools[c.rarity] = [];
	_rarityPools[c.rarity].push(c);
}
const _rarityFallback = _rarityPools[1] || [];

export function batchRandomScout(n: number): Uma[] {
	const picks: Uma[] = [];
	const newIds: string[] = [];
	...
	for (let i = 0; i < n; i++) {
		...
		const pool = _rarityPools[rarity];
		const source = pool && pool.length > 0 ? pool : _rarityFallback;
		const pick = source[Math.floor(Math.random() * source.length)];
		picks.push(pick);
		if (!current.includes(pick.id)) newIds.push(pick.id);
	}
	roster.update((r) => {
		const s = new Set(r);
...
const _supportRarityPools: Record<number, SupportCard[]> = {};
for (const c of supportCards) {
	if (!_supportRarityPools[c.rarity]) _supportRarityPools[c.rarity] = [];
	_supportRarityPools[c.rarity].push(c);
}
const _supportRarityFallback = _supportRarityPools[1] || [];

export function batchRandomSupportScout(n: number): SupportCard[] {
	const picks: SupportCard[] = [];
	const newIds: string[] = [];
	const current = get(supportRoster);
	for (let i = 0; i < n; i++) {
		sPullCount++;
		sPullSinceTwo++;
		let rarity: number;
...
export async function exportSave(): Promise<string> {
	const raw = JSON.stringify({
		version: 1,
		roster: get(roster),
		supportRoster: get(supportRoster),
		pity: { pullCount, pullSinceTwo, sPullCount, sPullSinceTwo }
	});
	return encrypt(raw);
}
...
export async function importSave(data: string): Promise<{ ok: boolean; error?: string }> {
	try {
		...
		if (parsed.supportRoster) supportRoster.set(parsed.supportRoster);
		if (parsed.pity) {
			pullCount = parsed.pity.pullCount ?? 0;
			pullSinceTwo = parsed.pity.pullSinceTwo ?? 0;
			sPullCount = parsed.pity.sPullCount ?? 0;
			sPullSinceTwo = parsed.pity.sPullSinceTwo ?? 0;
			savePity();
		}
		return { ok: true };
	} catch {
